...
public class ChatPlugin
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        CopilotChatMessage chatMessage = await this.GetChatResponseSimpleAsync(chatId, userId, chatContext, newUserMessage, language, cancellationToken);
    ...
    /// <summary>
    /// Generates the OData query based on the user request.
    /// </summary>
    /// <param name="chatContext">The SKContext.</param>
    /// <param name="userRequest">The user request in natural language.</param>
    /// <param name="language">The input language.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The OData query generated by the LLM.</returns>
    private async Task<string?> GenerateODataQueryAsync(SKContext chatContext, string userRequest, string language, CancellationToken cancellationToken)
    {
    ...
    /// <summary>
    /// Executes the OData query on the search index (Azure AI Search).
    /// </summary>
    /// <param name="query">The OData query.</param>
    /// <returns>The data returned by Azure AI Search.</returns>
    [SKFunction, Description("Execute the OData Query on the search index")]
    public async Task<string> ExecuteODataQueryAsync([Description("The OData query")] string query)
    {
        var client = this._httpClientFactory.CreateClient("ExecuteODataQuery");
        client.DefaultRequestHeaders.Add("api-key", this._azureAISearchOptions.APIKey);
        
        // TODO: Sanitize the OData query
        var response = await client.GetAsync(new Uri($"{_azureAISearchOptions.Endpoint}/indexes('{_azureAISearchOptions.IndexName}')/docs?{query}&api-version=2023-11-01"));
    ...
    private async Task<CopilotChatMessage> GetChatResponseSimpleAsync(string chatId, string userId, SKContext chatContext, CopilotChatMessage userMessage, string language, CancellationToken cancellationToken)
    {
        ...
        // Generate the OData query using a Semantic Function
        var query = await GenerateODataQueryAsync(chatContext, userMessage.Content, language, cancellationToken);

        if (!query.Contains("ERROR"))
        {
            await this.UpdateBotResponseStatusOnClientAsync(chatId, Resource.QuerySearchIndex, cancellationToken);

            // Execute the OData query on the search index (Azure AI Search)
            var data = await ExecuteODataQueryAsync(query);
            ...
            if (!String.IsNullOrEmpty(data))
            {
                ...
                if (jsonData != null && jsonData is JsonArray && jsonData.AsArray().Count > 0)
                {
                    ...
                    // Format the data returned by Azure AI Search
                    ...
                    // Create a new chat to generate the response
