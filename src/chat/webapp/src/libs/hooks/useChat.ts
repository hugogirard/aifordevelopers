...
export const useChat = () => {
    ...
    const getChatUserById = (id: string, chatId: string, users: IChatUser[]) => {
        if (id === `${chatId}-bot` || id.toLocaleLowerCase() === 'bot') {
            const botProfile = Constants.bot.profile;
            botProfile.fullName = TranslationFunc("Copilot");
            return botProfile;
        }
    ...
    const createChat = async () => {
        const chatTitle = TranslationFunc("Copilot") + ` @ ${new Date().toLocaleString()}`;
        try {
            await chatService
...
export function getFriendlyChatName(convo: ChatState): string {
    ...
    const autoGeneratedTitleRegex = new RegExp(TranslationFunc("Copilot") + " @ [0-9]{1,2}\/[0-9]{1,2}\/[0-9]{1,4}, [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2} [A,P]M");
    ...
    const friendlyTitle = autoGeneratedTitleRegex.test(convo.title)
        ? firstUserMessage?.content ?? TranslationFunc("NewChat")
        : convo.title;
